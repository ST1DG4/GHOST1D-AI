...
# Muestra todo el historial de chat
...
# Obtiene la entrada del usuario (de texto o de voz)
prompt = st.chat_input("Escribe tu pregunta o pídemelo...")
if st.session_state.get('user_input'):
    prompt = st.session_state.pop('user_input')

# Si hay una nueva entrada, se procesa todo el ciclo de respuesta AHORA
if prompt:
    ...
    # Crea un placeholder para la respuesta del asistente
    with st.chat_message("assistant", avatar=st.session_state.get('assistant_avatar', '🤖')):
        with st.spinner("GhoStid AI está pensando..."):
            # Lógica del agente para obtener la respuesta
            ...
            # Muestra el texto de la respuesta INMEDIATAMENTE
            ...
            if st.session_state.voice_enabled and st.session_state.voices_map:
                ...
                if speakable_text:
                    ...
                    if audio_bytes:
                        # Muestra el audio INMEDIATAMENTE
